 ...
 * @version
 * @desc
 * @edited by Chenyifei Aug 15th
 * @在南昌自动控制的基础上去除了水源开关环节，同时结合本地电源状况加入了延时发送命令。
 */
...
class Irrigation {
    ...
    public function run() {
        $data = $this->_db->where("net_status=7")->where("device_id like '100%'")->get("device");
        $end = 0;
        foreach ($data as $row) {
